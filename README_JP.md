...
```
「我が生には涯（かぎり）有るも、��には涯無し。涯有るを以て涯無きに随（したが）うは、殆（あやう）きのみ！」—— 『荘子・内篇・養生主第三』
```
...
wiseflow add-on は現在、openclaw に以下の3つのコア機能強化を提供しています：

### 1. アンチ検出ブラウザ

openclaw に内蔵された Playwright を [Patchright](https://github.com/Kaliiiiiiiiii-Vinyzu/patchright)（Playwright の検出回避フォーク）に置き換え、自動化ブラウザがターゲットサイトに検出・ブロックされる可能性を大幅に低減します。
...
openclaw に内蔵された `web_search` をより強力な検索機能に置き換えます。市場の同等ソリューションと比較して、スマート検索には3つの主要な優位性があります：

- **完全無料、API キー不要**：サードパーティの検索 API に依存せず、ゼロコストで利用可能
- **リアルタイム検索、最高の鮮度**：ブラウザを直接ターゲットページや主要なソーシャルメディアプラットフォーム（Weibo、Twitter/X、Reddit など）に誘導し、最新公開コンテンツを即座に取得
