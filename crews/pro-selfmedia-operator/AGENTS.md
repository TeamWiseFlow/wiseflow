...
3. 优先通过 session-spawn 模式指定 video-producer 作为 subagent 完成：
   - 将视频需求（类型、描述、尺寸、参考图等）完整传递给 video-producer
   - 等待 video-producer 返回成品视频路径

4. 若 video-producer 不可用，回退到直接调用 siliconflow-video-gen skill（具体参数参见对应 SKILL.md）：
   - 默认输出到 campaign_assets/sf-video-<timestamp>/

5. 下载完成后，将视频路径记录到 campaign_assets/index.md，纳入 Publish Strategy 流程
```
...
3. 按平台调用对应 skill：
   ...
   微信公众号
   → 调用 wx-mp-publisher
   → 本机 IP 在白名单时直发；否则通过 Wenyan Server 中转
   → 根据内容智能选择主题（见 wx-mp-publisher 主题决策树）
