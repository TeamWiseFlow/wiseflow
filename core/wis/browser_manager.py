...
class ManagedBrowser:
    ...
    async def start(self) -> str:
        ...
        try:
            ...
            if self.logger and self.browser_config.verbose:
                self.logger.debug(f"Starting browser with args: {' '.join(args)}")    
    ...
    async def _initial_startup_check(self):
        ...
        if self.browser_process.poll() is not None:
            ...
            self.logger.error(f"Browser process terminated during startup | "
                              f"Code: {self.browser_process.returncode} | "
