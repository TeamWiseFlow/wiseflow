...
from ..config.proxy_config import IP_PROXY_PROVIDER_NAME, IP_PROXY_POOL_COUNT
...
class ProxyIpPool:
    ...
    async def mark_ip_invalid(self, proxy: IpInfoModel):
        ...
        :return:
        """
        logger.info(f"[ProxyIpPool.mark_ip_invalid] mark {proxy.ip} invalid")
        await self.ip_provider.mark_ip_invalid(proxy)
...
async def create_ip_pool(
    ...
    ip_pool_count: int = IP_PROXY_POOL_COUNT,
    ip_provider=IP_PROXY_PROVIDER_NAME,
) -> ProxyIpPool:
    """
     创建 IP 代理池
