from ..mc_commen.tools.time_util import get_date_str_from_unix_time
from typing import Dict, List


def update_kuaishou_video(video_item: Dict, keyword: str = "") -> Dict:
...
def update_ks_video_comment(comment_items: List[Dict]) -> str:
    ...
    for comment_item in comment_items:
        create_time = get_date_str_from_unix_time(comment_item.get("timestamp"))
        ...
        for sub_comment in sub_comments:
            create_time = get_date_str_from_unix_time(sub_comment.get("timestamp"))
            ...
            comment_str += f"\t用户 {user_id} 于 {create_time} 回复用户 {replyto} :\n\t{content}\n\t获赞：{like_count}\n"
