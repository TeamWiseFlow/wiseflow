...
If audio is present, it replaces the video's audio track.
...
def find_files(directory: str, extensions: set[str], exclude: set[str] | None = None) -> list[str]:
    """Find files matching given extensions in deterministic order."""
...
def _normalize_and_concat_batch(video_files: list[str], width: int, height: int,
                                output_path: str, tmp_dir: str, batch_size: int = 3) -> str:
    """Normalize a batch of videos via filter_complex, then concat with ffmpeg concat demuxer.
    ...
    # Step 1: normalize each video individually (scale/pad/fps/format)
...
def assemble_multiple_videos(video_files: list[str], audio_file: str | None, output_path: str) -> None:
    ...
    try:
        # Step 1: normalize + concat video-only
        video_only = os.path.join(tmp_dir, "video_only.mp4")
        _normalize_and_concat_batch(video_files, width, height, video_only, tmp_dir)
        ...
        if audio_file:
            ...
            # Just rename the video-only file to output
