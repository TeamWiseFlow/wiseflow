...
**模型层级选择原则**：
- `strong`（Opus）：需要深度推理的模块，如整体架构设计、核心分析综合、验收评估
- `default`：执行型模块，如内容生产、数据整理、格式转换
...
| A5 | 指派��部 crew | 是否有模块错误地指派了 external crew？ |
...
> **注意**：自我评估存在天然的乐观偏差。对于高质量要求的任务，可在此阶段 spawn 独立的 evaluator agent（指定 `strong` 模型），专门负责批判性评估，避免"自产自评"。
...
| P5 | 自评偏乐观 | 低质量成果被误判为合格 | 重要任务 spawn 独立 evaluator；Phase 7 评分严格，宁缺毋滥 |
